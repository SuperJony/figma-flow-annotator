...
- `kind`: written on project-created visual roots. Values include `annotation-card`, `annotation-badge`, and `flow-connector`.
...
- `validationIndex`: bounded validation data on the current page, containing `schemaVersion` and known project node ids used by validation, stale-index cleanup, and explicit repair paths.
