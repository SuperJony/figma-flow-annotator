...
test("validates Annotation bindings and locates validation issue nodes without shared report data", async () => {
  ...
  cardMissingBadge.setSharedPluginData(
    namespace,
    "annotation",
    JSON.stringify({
      schemaVersion: 1,
  ...
  const missingBodyIssue = reportMessage.report.issues.find(
    (issue) => issue.code === "annotation-missing-body",
  );
  messages.length = 0;
  globalThis.figma.ui.onmessage({
    type: "locate-validation-issue",
    issueId: missingBodyIssue.id,
  });
  await flushPluginMessage(messages);
...
test("validates Flow Connector references, locates issues, and cleans stale indexes", async () => {
  ...
  const staleIssue = reportMessage.report.issues.find(
    (issue) => issue.code === "connector-reverse-index-stale",
  );
  messages.length = 0;
  ...
  const cleanReport = messages
    .filter((message) => message.type === "validation-report")
    .at(-1).report;
...
test("validates route, label, and trunk connector issues without shared report data", async () => {
  ...
  assert.equal(
    report.issues.some((issue) => issue.code === "connector-route-crosses-obstacle"),
    true,
  );
  assert.equal(
    report.issues.some((issue) => issue.code === "connector-routing-failure"),
    true,
  );
  assert.equal(
    report.issues.some((issue) => issue.code === "flow-action-label-overlap"),
    true,
  );
  assert.equal(
    report.issues.some((issue) => issue.code === "connector-route-refreshable"),
    true,
  );
  assert.equal(
    report.issues.some((issue) => issue.code === "connector-trunk-missing"),
    true,
  );
  assert.equal(page.getSharedPluginData(namespace, "validationReport"), "");
  ...
  assert.deepEqual(
    page.selection.map((node) => node.id),
    ["connector-trunk-root-a", "connector-trunk-root-b"],
  );
  assert.deepEqual(
    scrollEvents.at(-1).map((node) => node.id),
    ["connector-trunk-root-a", "connector-trunk-root-b"],
